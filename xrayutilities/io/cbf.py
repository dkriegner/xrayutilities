...
class CBFFile(object):
    def __init__(self,fname,nxkey="X-Binary-Size-Fastest-Dimension",nykey="X-Binary-Size-Second-Dimension",dtkey="DataType",path=None):
        """
    ...
    def Save2HDF5(self,h5,group="/",comp=True):
        ...
        ca_name = os.path.splitext(ca_name)[0] # perform a second time for case of .cbf.gz files
...
class CBFDirectory(object):
    ...
    def __init__(self,datapath,ext="cbf",**keyargs):
        ...
        optional keyword arguments:
        ext .................. extension of the ccd files in the datapath (default: "cbf")
        ...
        if config.VERBOSITY >= config.INFO_ALL:
            print("XU.io.CBFDirectory: %d files found in %s" %(len(self.files),self.datapath))
    ...
    def Save2HDF5(self,h5,group="",comp=True):
        """
        Saves the data stored in the CBF files in the specified directory
