...
class TestPseudomorphic(unittest.TestCase):
    ...
    def test_pseudomoprhic001(self):
        ...
        asub = (self.mA.lattice.a + self.mA.lattice.b) / 2.0
        abulk = (self.mB.lattice.a + self.mB.lattice.b) / 2.0
    ...
    def test_pseudomorphic111(self):
        ...
        def get_inplane111(lay):
            ...
            return (
                xu.math.VecNorm(lay.GetPoint(1, 1, -2)) / math.sqrt(6)
                + xu.math.VecNorm(lay.GetPoint(1, -1, 0)) / math.sqrt(2)
            ) / 2
        ...
        eperp = (
            -epar
            * (2 * self.mB.c11 + 4 * self.mB.c12 - 4 * self.mB.c44)
        ...
        self.assertAlmostEqual(
            ...
            2 * pi / (xu.math.VecNorm(self.mB.Q(1, 1, -2)) / (1 + epar)),
            places=12,
        )
        self.assertAlmostEqual(
            mBpseudo.planeDistance(1, 1, 1),
            2 * pi / (xu.math.VecNorm(self.mB.Q(1, 1, 1)) / (1 + eperp)),
            places=12,
        )
