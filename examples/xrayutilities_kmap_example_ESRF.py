...
roi = [0,516,50,300] # region of interest on the detector; useful to reduce the amount of data
...
hxrd.Ang2Q.init_area('z-','y+',cch1=200.07,cch2=297.75,Nch1=516,Nch2=516, pwidth1=9.4489e-05,pwidth2=9.4452e-05,distance=1.,detrot=-0.801,tiltazimuth=30.3,tilt=1.611, roi=roi)
...
# specfile, scannumbers, nx,ny, motornames, optional column names (ID01 values are default)
fss = xu.io.FastScanSeries(sample,scannr,nx,ny,'Mu','Eta','Nu','Delta',xmotor='adcY',ymotor='adcX',ccdnr='imgnr',path=specdir)
# retrace clean all scans
fss.retrace_clean()
# align different scans (misalignment needs to be determined manually or if your application allows also by a 2d correllation code
...
g = fss.gridRSM(posx,posy,qnx,qny,qnz,qconv,ccdfiletmp,path='',roi=roi,nav=nav,typ='index',filterfunc=hotpixelkill,UB=U)
# with typ='real' the position should be real space coordinates. with typ='index' the posx,y should specify indices within the range(nx) range(ny)
...
# Note: if you instead want to grid in two dimensions you can decrease one of qnx,qny or qnz to 1 and interprate the data in 2D
